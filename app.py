...
def aqi():
    ...
    for x in range(len(data)):
        if data[x]['County'] == '臺北市': 
            if data[x]['Status'] == '設備維護':
                content += '{}測站目前設備維護中\n\n'.format(data[x]['SiteName'])
                continue         
            content += '{}目前的AQI指數為:{}\n空氣品質為:{}\n\n'.format(data[x]['SiteName'],data[x]['AQI'],data[x]['Status'])

    return content

def freeway():
    target_url = 'http://pda.freeway.gov.tw/section/10050'
    print('Start parsing freeway data ...')
    rs =requests.session()
    res = rs.get(target_url, verify=False)
    res.encoding = 'utf-8'
    soup = BeautifulSoup(res.text, 'html.parser')
    ...
    speed = soup.find_all('div', class_='speed')
    for s in speed:
        content += '{}\n'.format(speed)
    return content
...
def technews():
    ...
    for index, data in enumerate(soup.select('article div h1.entry-title a')):
        ...
        content += '{}\n{}\n\n'.format(title, link)
    return content
#post-359472 > div > header > table > tbody > tr:nth-child(1) > td > h1 > a
